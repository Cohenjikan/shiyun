...
| **UF-14** | 白居易《长恨歌》「侍儿扶起娇无力」讹作「**待**儿」(用户反馈第三批,2026-07-24) | upstream-typo | ✅已修(corpus a611d41,首句锚唯一 applied=1) |
...
| **UF-12** | 张居正缺《咏竹》《闻警》《述怀》(用户反馈第二批) | upstream-missing | ✅咏竹已补(轶闻类注记)·闻警/述怀考据不过不收(孤证/仅节选) |
| **UF-13** | 「竟」显示成「竞」(用户反馈第二批) | not-an-error | 🧊 owner 确认误报,关闭(2026-07-09) |
| **UF-14** | 白居易《长恨歌》「待儿扶起」讹字(用户反馈第三批) | upstream-typo | ✅已修(侍儿,据《全唐诗》通行本;全库唯一孤例,库内化用句/当代引文均作正字为旁证) |
...
- **2026-07-24**(用户反馈勘误第三批,UF-14):白居易《长恨歌》「**待**儿扶起娇无力」→「**侍**儿」——上游 Werneror 唐.csv 孤例错字(全库 97 万行「待儿扶起娇无力」唯一命中;旁证:库内段克己/姚勉化用句、当代轩辕轼轲同名诗引文均作正字)。corpus `curated/corrections.jsonl` 订正(首句锚「汉皇重色思倾国」全库唯一,applied=1/findMiss=0,corrected_from 全文存证,corpus **a611d41**,VALIDATE PASS 970,325 行不变;poets.jsonl 白居易 has_curated→true)+ shiyun main 全链重建(build-data→lines/search/fuzzy;charset FROZEN 逐字节不变 N=22219、manifest/poets.index/gifts 零 diff、charset-skip=0、poems 871,268/poets 32,654/edges 5,051 与 canonical 基线一致)。错字原扩散三层:poems 正文 + lines 名句旁车(白居易系名家,错句可上探诗面)+ linesf;修后残留仅 `_v1_backup` 历史备份(有意)。
